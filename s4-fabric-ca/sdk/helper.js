...
async function getClientForOrg (userorg, username) {
    ...
    const USER_ID = username+"@org1.default.svc.cluster.local"

    const userIdentity = await wallet.export(USER_ID);
    logger.debug(USER_ID, 'retrieved...');
    ...
    logger.debug('Intilizsing Gateway...');
    const gateway = new Gateway();
    await gateway.connect(connectionProfile, { wallet, identity: USER_ID, discovery: { enabled: true, asLocalhost: true } });


    const client = gateway.getClient();
    console.log('client retrived...')
    
   // const adminUser = await client.getUserContext(USER_ID, false);
